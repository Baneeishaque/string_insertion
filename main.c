#include <stdio.h>

int main() {

    int i, j, count = 0, pos, flag = 0;
    char s1[100], s2[10], s3[100];
    char *ptr1, *ptr2, *ptr3;

    printf("\nPlease Enter the String: ");
    //    welcome to Hyderabad international airport
    scanf(" %[^\n]s", s1);
    printf("\nEnter the Position For Insertion of String: ");
    //    3
    scanf("%d", &pos);
    printf("\nEnter the String to enter at Position %d:",pos);
    //    RajivGandhi
    scanf(" %[^\n]s", s2);

    ptr1 = s1;
    ptr3 = s3;

    /*COPYING THE GIVEN STRING TO NEW ARRAY AND INSERTING THE STRING IN NEW ARRAY*/
    for (i = 0, j = 0; *ptr1 != '\0'; ptr1++, i++, j++, ptr3++) {

        s3[j] = s1[i];

        if (*ptr1 == ' ' && flag != 1)
            ++count;

        if (flag != 1 && count == pos - 1) {

            flag = 1;

            for (ptr2 = s2; *ptr2 != '\0'; ptr2++) {

                s3[++j] = *ptr2;
                ptr3++;
            }

            s3[++j] = ' ';
            ptr3++;
        }
    }

    s3[j] = '\0';
    //TODO : Fix Output String
    printf("\nString After Modification: %s\n", s3);
    //    welcome to Hyderabad RajivGandhi international airport

    return 0;
}
